...
def is_local():
    ...
    return hostname == "Ankithas-MacBook-Air" or os.path.exists("online_course_recommendation_v2.xlsx")
...
def load_models():
    ...
    # Load different dataset depending on environment
    if is_local():
        df = pd.read_excel("online_course_recommendation_v2.xlsx")
    else:
        df = pd.read_csv("df_encoded.csv")  # fallback for deployment
...
def hybrid_recommend(user_id, top_n=5, difficulty=None, certification=None):
    ...
    # Merge with df for human-readable fields
